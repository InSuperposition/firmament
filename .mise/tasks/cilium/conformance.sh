#!/usr/bin/env bash
#MISE description="Run the Cilium connectivity suite against the cluster, with Hubble flow validation, checking only logs written during the tests, then remove its test workloads (slow; deploys test workloads)"
...
# passes, so the forward must be listening before the suite starts. With
# Hubble reachable, `warning` fails the run on any flow mismatch, as
# `strict` does; the two differ only when Hubble cannot be reached.
